...
相機吐出來的是 YUV `byte[]`，但 AI SDK 介面偏偏要求傳入 `Bitmap`。如果每幀呼叫 `Bitmap.createBitmap()` 或 `bitmap.copy()`，每秒會額外產生 36MB 的 Bitmap 垃圾。

我們在開機時**只 new 一張固定的 Bitmap 鐵碗**，推論時透過底層記憶體直接覆寫像素，達成 0 分配：

```java
// 開機只 new 一次 Bitmap 鐵碗
Bitmap reusableBitmap = Bitmap.createBitmap(640, 480, Bitmap.Config.ARGB_8888);
ByteBuffer bufferWrapper = ByteBuffer.allocateDirect(640 * 480 * 4);

public void processFrame(byte[] ironBowlBytes) {
    bufferWrapper.rewind();
    bufferWrapper.put(ironBowlBytes);
    bufferWrapper.rewind();

    // 直接 memcpy 刷入既有 Bitmap 的 Native 記憶體位址（0 記憶體分配、0 GC）
    reusableBitmap.copyPixelsFromBuffer(bufferWrapper);
...
整台設備在推論循環中只有一張 Bitmap 實體，徹底消滅 GC 壓力。
...
4. **記憶體抽脂戰果**：歷經鐵碗循環、Native 覆寫與 DirectBuffer 零拷貝這五大手術，相機管線徹底告別了開機預設狂吞 950MB 的全域 ION/Gralloc 黑洞，最終在主進程中被精準壓制在穩態的 120MB，為後續 AI 模型奪回了整整 830MB 的實體空間。
