...
1. 限制資料的視野：利用synchronized，保護共享物件的臨界區域。把資料封裝的原則謹記在心，並嚴格現職共享資料的存取次數。
 2. 使用資料的副本：多執行緒執行時存取時，可以盡可能讓共享物件複製多分，保持唯獨。CopyOnWriteArrayList 的機制是在每次寫入操作（add / set / remove）時，直接複製一份全新的內部陣列來保障讀取執行緒的安全性。它是一種『寫時複製（Copy-on-Write）』策略，適用於讀多寫少的場景，並非最後將多份資料合併的模式。
